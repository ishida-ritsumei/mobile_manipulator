...
def blended_motions(axis_num):
	...
	req_lineb.mvvelo = 10 # mm/s
	req_lineb.mvacc = 10 # mm/s^2
	req_lineb.mvtime = 0 
	req_lineb.mvradii = 25 # blending radius: 25 mm
	position_list = [[230, 0, 280, 3.14, 1.57, 0], ]#[100, 0, 680, 0, 0, 3.14]]  #[[300, 200, 100, -3.14, 0, 0], [500, 200, 100, -3.14, 0, 0], [500, -200, 100, -3.14, 0, 0], [300, -200, 100, -3.14, 0, 0]]
	
	req_jointb = MoveRequest() 
	req_jointb.mvvelo = 0.3 # rad/s
	req_jointb.mvacc = 20 # rad/s^2
	req_jointb.mvtime = 0 
...
if __name__ == "__main__":
	...
	# Gripperの設定を行うサービスを呼び出すクライアントを作成
	# config_client = rospy.ServiceProxy('/xarm/gripper_config', GripperConfig)
	# Gripperの制御を行うサービスを呼び出すクライアントを作成
	# move_client = rospy.ServiceProxy('/xarm/gripper_move', GripperMove)

	try:
		# Gripperを開く
		# config_client(0)  # グリッパーを設定
		# move_client(1)    # グリッパーを開く
		# グリッパーを半分だけ開く
		# config_client(0.5)  # グリッパーを設定（0.5は開度を表します）
		# move_client(1)      # グリッパーを開く
		motion_en(8,1)
		set_mode(0)
		set_state(0)
