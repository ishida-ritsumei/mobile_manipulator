...
for i in range(int(10)):
    ...
    if arm.error_code == 0 and not params['quit']:
        code = arm.set_servo_angle(angle=[0.0, -45.0, 0.0, 45.0, 0.0], speed=params['angle_speed'], mvacc=params['angle_acc'], wait=True, radius=None)
        if code != 0:
            params['quit'] = True
            pprint('set_servo_angle, code={}'.format(code))
    if arm.error_code == 0 and not params['quit']:
        code = arm.set_servo_angle(angle=[-360.0, 0.0, -180.0, 0.0, 90.0], speed=params['angle_speed'], mvacc=params['angle_acc'], wait=True, radius=None)
        if code != 0:
            params['quit'] = True
            pprint('set_servo_angle, code={}'.format(code))
